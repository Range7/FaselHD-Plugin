// Cee Scraper for Nuvio
// Cee - STRICT 1080p ONLY | Exact Title Matching | All Languages
// Same API as Cinemana, different domain
...
var API_BASE = "https://cee.buzz/api/android";
var MAIN_URL = "https://cee.buzz";
...
function getSearchQueries(meta) {
  ...
  console.log("[Cee] Content language: " + lang + " | Queries: " + queries.join(" | "));
  return queries;
}

// ─── Search Cee ──────────────────────────────────────────
function searchCeeSingle(query, year, type) {
...
function searchCeeAll(queries, year, mediaType) {
  return __async(this, null, function* () {
    ...
    for (var i = 0; i < queries.length; i++) {
      var query = queries[i];
      console.log("[Cee] Searching with: '" + query + "' (type: " + type + ")");

      var results = yield searchCeeSingle(query, year, type).catch(function() { return []; });
    ...
    if (allResults.length === 0) {
      console.log("[Cee] No results with year, trying without year...");
      for (var j = 0; j < queries.length; j++) {
        var q = queries[j];
        var res = yield searchCeeSingle(q, null, type).catch(function() { return []; });
...
function findExactMatch(results, meta, mediaType) {
  ...
  console.log("[Cee] Looking for EXACT match. Queries: " + queries.join(" | "));
  ...
  if (bestScore < 80) {
    console.log("[Cee] REJECTED — best score: " + bestScore + " (need 80+ for exact match)");
    return null;
  }

  console.log("[Cee] EXACT MATCH: '" + bestMatch.title + "' (score: " + bestScore + ")");
  return bestMatch;
}
...
function getStreamLinks(videoId) {
  ...
  return safeFetch(streamsUrl).then(function(r) {
    ...
    if (streams.length === 0) {
      console.log("[Cee] No 1080p streams found.");
      return [];
    }

    console.log("[Cee] Found " + streams.length + " x 1080p stream(s).");
    return streams;
  });
}
...
function getStreams(tmdbId, mediaType, season, episode) {
  return __async(this, null, function* () {
    ...
    console.log("[Cee] === " + type + "/" + tmdbId + " S" + (season || 1) + "E" + (episode || 1) + " ===");

    try {
      var meta = yield getTmdbMeta(tmdbId, mediaType);
      console.log("[Cee] TMDB: '" + meta.title + "' | Original: '" + meta.originalTitle + "' | Lang: " + meta.originalLang + " | Year: " + meta.year);

      var queries = getSearchQueries(meta);
      var searchResults = yield searchCeeAll(queries, meta.year, mediaType);

      if (!searchResults || searchResults.length === 0) {
        console.log("[Cee] No search results found.");
        return [];
      }

      var match = findExactMatch(searchResults, meta, mediaType);
      if (!match) {
        console.log("[Cee] No exact match found. Content may not exist on Cee.");
        return [];
      }

      var videoId = match.id;
      console.log("[Cee] Exact match ID: " + videoId + " | Title: " + match.title);

      if (mediaType === "tv" || mediaType === "anime") {
        var epId = yield getEpisodes(videoId, season || 1, episode || 1);
        if (epId) {
          console.log("[Cee] Episode ID: " + epId);
          videoId = epId;
        } else {
          console.log("[Cee] Episode not found, using series ID.");
        }
      }

      var streams = yield getStreamLinks(videoId);
      if (!streams || streams.length === 0) {
        console.log("[Cee] No 1080p streams available for this content.");
        return [];
      }
      ...
      console.log("[Cee] === Done: " + finalStreams.length + " x 1080p streams in " + (Date.now() - t0) + "ms ===");
      return finalStreams;

    } catch (error) {
      console.log("[Cee] Error: " + error.message);
      return [];
    }
  });
}
